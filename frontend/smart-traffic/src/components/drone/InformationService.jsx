...
class InformationService {
    static addInformation(newData) {
      return fetch(`http://localhost:8080/drone/add`, {
        method: 'POST',
        headers: {
      ...
      .then(response => {
        if (!response.ok) {
            console.log('Failed to add new drone')
          throw new Error('Failed to add new drone');
        }
    ...
    static updateInformation(itemId, updatedData) {
      return fetch(`http://localhost:8080/cctv/${itemId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
